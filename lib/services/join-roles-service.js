...
class JoinRolesService extends Service {
  ...
  addUserToRole(member, role) {
    return of('').pipe(
      ...
      flatMap(allowed => !allowed
        ? throwError(new NonJoinableRoleError("Role can't be joined"))
        : of(''),
      ),
      flatMap(() => member.roles.has(role.id)
        ? throwError(new JoinRoleError("User already has role"))
        : of(''),
      ),
  ...
  removeUserFromRole(member, role) {
    return of('').pipe(
      ...
      flatMap(() => !member.roles.has(role.id)
        ? throwError(new LeaveRoleError("User is not in the role"))
        : of(''),
      ),
